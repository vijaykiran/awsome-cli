...
impl DynamoDbService {
    ...
    pub async fn describe_table(&self, table_name: &str) -> Result<Vec<(String, String)>> {
        ...
        if let Some(table) = resp.table {
            ...
            details.push(("Size".to_string(), format_size(table.table_size_bytes.unwrap_or(0))));
            ...
            if let Some(gsi) = table.global_secondary_indexes {
                if !gsi.is_empty() {
                    ...
                    // Header
                    details.push(("".to_string(), format!("{:<30} {:<15} {:<10}", "Index Name", "Status", "Items")));
                    details.push(("".to_string(), "-".repeat(60)));
                    
                    // Index rows
                    for index in gsi.iter() {
                        let name = index.index_name.as_deref().unwrap_or("?");
                        let status = index.index_status.as_ref().map(|s| s.as_str()).unwrap_or("?");
                        let items = index.item_count.unwrap_or(0);
                        details.push(("".to_string(), format!("{:<30} {:<15} {:<10}", name, status, items)));
                    }
                }
            }
            ...
            if let Some(lsi) = table.local_secondary_indexes {
                if !lsi.is_empty() {
                    ...
                    // Header
                    details.push(("".to_string(), format!("{:<30} {:<10}", "Index Name", "Items")));
    ...
    pub fn format_table_list(tables: &[(String, String, String, String)]) -> (Vec<String>, Vec<DynamoDbItem>) {
        if tables.is_empty() {
            return (vec!["No DynamoDB Tables found".to_string()], vec![DynamoDbItem::Header]);
        }

        let max_name_len = tables.iter()
        ...
        let header = format!(
            "{:<width$}  {:<10}  {:<12}  Size",
            "Table Name", "Status", "Items",
            width = max_name_len
        );
        let separator = format!("{}", "-".repeat(max_name_len + 40));
